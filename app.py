...
dark_mode = st.sidebar.checkbox("🌙 Night Mode", value=st.session_state.dark_mode, key="dark_mode_toggle")
st.session_state.dark_mode = dark_mode

css_background(dark_mode=dark_mode)
...
if page == "📈 Overview & EDA":
    ...
    with col1:
        ...
        fig2 = px.box(dff, x="Stress_Level", y="Sleep_Hours_Per_Day", title="Sleep by Stress Level")
    ...
    with col2:
        ...
        fig4 = px.imshow(corr, text_auto=True, title="Numeric feature correlation")
    ...
    if not numeric_features_list:
        ...
        # Feature selection
        default_features = numeric_features_list[:4] if len(numeric_features_list) >= 4 else numeric_features_list
        ...
        if not chosen:
            ...
            # Residual plot
            fig_res = go.Figure()
            fig_res.add_trace(go.Scatter(x=y_test, y=y_pred - y_test, mode="markers", name="Residuals", marker=dict(size=8, color="#3a0ca3")))
            fig_res.update_layout(title="Residual Plot", xaxis_title="Actual CGPA", yaxis_title="Residual", height=400)
            ...
            for idx, feature in enumerate(chosen):
                ...
                with pred_cols[col_idx]:
                    ...
                    pred_inputs[feature] = st.slider(
                        ...
                        value=med_val,
                        step=0.01,
                        key=f"reg_slider_cgpa_{feature}_{id(chosen)}"
                    )
    ...
    if not numeric_features_list:
        ...
        if not features:
            ...
            col3.metric("F1 Score (weighted)", f"{f1:.3f}")
